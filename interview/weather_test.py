...
def test_process_events_snapshot_control():
    ...
    Expected: 3 samples + snapshot output + control message = 5 total.
    Snapshot should aggregate temperature ranges by station.
    """
    events = [
        {"type": "sample", "stationName": "Station1", "temperature": 25.0, "timestamp": 1000},
...
def test_process_events_reset_control():
    ...
    Reset clears data, so snapshot after reset produces no output.
    """
    events = [
        {"type": "sample", "stationName": "Station1", "temperature": 25.0, "timestamp": 1000},
    ...
    assert result[2] == events[1]
    assert result[3] == events[2]

def test_process_events_reset_no_timestamp():
    """Test reset control message with no prior timestamp.
    
    Reset with no prior data should use timestamp 0.
    """
...
def test_process_events_is_generator():
    ...
    Verify generator protocol for lazy evaluation requirement.
    """
